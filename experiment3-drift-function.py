...
for j, epsilon in enumerate([0.05, 0.1]):
    ...
    plt.plot(x, [V_tilde_prime(xi, sigma) for xi in x], label="Exact $b$", linewidth=3, c='C0')
    plt.plot(x, np.sum(A_array[int(10/(T/n)), :, None]*np.array([x**i for i in range(r+1)]), axis=0), label="$\widetilde b$ ($T=10$)", linewidth=3, c='C1', alpha=0.15)
    plt.plot(x, np.sum(A_array[int(100/(T/n)), :, None]*np.array([x**i for i in range(r+1)]), axis=0), label="$\widetilde b$ ($T=100$)", linewidth=3, c='C1', alpha=0.6)
    plt.plot(x, np.sum(A_new[:, None]*np.array([x**i for i in range(r+1)]), axis=0), label="$\widetilde b$ ($T=1000$)", linewidth=3, c='C1', alpha=1)
    ...
    if j == 0:
        plt.savefig(f'./Paper Figures/experiment3-exponential-42-time-05.pdf', bbox_inches='tight')
        with open(f"./Paper Data/experiment3-exponential-42-time-05.npy", 'wb') as f:
            np.save(f, A_new)
    else:
        plt.savefig(f'./Paper Figures/experiment3-exponential-42-time-1.pdf', bbox_inches='tight')
        with open(f"./Paper Data/experiment3-exponential-42-time-1.npy", 'wb') as f:
            np.save(f, A_new)
    plt.clf()