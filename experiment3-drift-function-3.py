...
print(np.min(Y), np.max(Y), np.min(Y_f), np.max(Y_f))

for r in [2, 3, 4]:
    #r = 2 # approximation order
    A_array = []
    A_old = np.zeros(r+1)
    A_new = np.zeros(r+1)
    ...
    plt.legend()
    if MOVING_AVERAGE:
        plt.savefig(f'./Paper Figures/experiment3-3-ma-42-{r}.pdf', bbox_inches='tight')
        with open(f"./Paper Data/experiment3-3-ma-42-{r}.npy", 'wb') as f:
            np.save(f, A_new)
    else:
        plt.savefig(f'./Paper Figures/experiment3-3-exponential-42-{r}.pdf', bbox_inches='tight')
        with open(f"./Paper Data/experiment3-3-exponential-42-{r}.npy", 'wb') as f:
            np.save(f, A_new)
    plt.clf()
